...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim):
        ...
        self.linear_up = nn.Linear(latent_dim, 512*80)
        self.relu = nn.ReLU()
        self.upsize5 = upsample_conv(input_channels=512, output_channels=256, size=(8, 9, 8), kernel_size=1)
        self.res_block5 = resnet_block(channels=256, kernel_size=3)
        self.upsize4 = upsample_conv(input_channels=256, output_channels=128, size=(15,17,15), kernel_size=1)
        self.res_block4 = resnet_block(channels=128, kernel_size=3)
        self.upsize3 = upsample_conv(input_channels=128, output_channels=64, size=(30,34,30), kernel_size=1)
        self.res_block3 = resnet_block(channels=64, kernel_size=3)
        self.upsize2 = upsample_conv(input_channels=64, output_channels=32, size=(59,68,59), kernel_size=1)
...
class VAE(nn.Module):
    def __init__(self, latent_dim=128):
        ...
        self.latent_dim = latent_dim
        self.z_mean = nn.Linear(512*80, latent_dim)
        self.z_log_sigma = nn.Linear(512*80, latent_dim)
